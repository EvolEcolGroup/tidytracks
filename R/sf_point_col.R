...
#'
#' This function creates a simple feature POINT geometry column from x and y
#' columns in a data frame. The resulting object is a `sf` geometry set that can
#' be used as an additional spatial column in an sf data frame, in addition to
...
#' @param crs An optional coordinate reference system (CRS) to assign to the
#'   geometry. This can be an EPSG code (numeric) or a proj4 string (character).
