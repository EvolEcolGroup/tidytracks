...
#' [hr_tt_ud_unwrap()] to restore the standard in-memory representation.
#'
#' @param x An `hr_tt_ud_tbl` with a `ud` list-column.
#' @returns A copy of `x` with a wrapped `ud` column.
#' @export
#' @examples
#' example_kde <- hr_tt_kde(example_tt)
#' wrapped_kde <- hr_tt_ud_wrap(example_kde)
hr_tt_ud_wrap <- function(x) {
  stopifnot_hr_tt_ud_tbl(x)
...
#'
#' Converts the `ud` column of an `hr_tt_ud_tbl` from a
...
#' with [hr_tt_ud_saveRDS()].
#' Functions that operate on `hr_tt_ud_tbl` objects unwrap their input
...
#' @examples
#' example_kde <- hr_tt_kde(example_tt)
#' unwrapped_kde <- hr_tt_ud_unwrap(hr_tt_ud_wrap(example_kde))
hr_tt_ud_unwrap <- function(x) {
  stopifnot_hr_tt_ud_tbl(x)
...
#' whose `hr_tt_ud_tbl` class was removed by dplyr grouping operations.
#'
...
#'
#' Wraps an `hr_tt_ud_tbl` before saving it with [base::saveRDS()]. This avoids
...
#' `hr_tt_ud_tbl` unwrap this column automatically; use [hr_tt_ud_unwrap()] to
...
stopifnot_hr_tt_ud_tbl <- function(x) {
  if (!inherits(x, "hr_tt_ud_tbl") && !inherits(x, "grouped_df")) {
    stop("x must be an hr_tt_ud_tbl or a grouped tibble")
  }
  if (!"ud" %in% names(x)) {
