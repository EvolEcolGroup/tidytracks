...
test_that("tt_read_data works with events CSV and meta CSV", {
  ...
  tt_write_data(example_tt, file_prefix = test_path, combined = FALSE)
  # check that we have an event file with columns track_id, date_time, X and Y
  event_file <- paste0(test_path, "_events.csv")
  ...
  meta_file <- paste0(test_path, "_metadata.csv")
  ...
  tt_write_data(example_tt, file_prefix = test_path, combined = TRUE)
  combined_file <- paste0(test_path, "_combined.csv")
