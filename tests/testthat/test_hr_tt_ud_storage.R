test_that("hr_tt_ud_wrap/unwrap preserve utilisation distributions", {
  ...
  wrapped_kde <- hr_tt_ud_wrap(boar_kde)
  ...
  expect_identical(hr_tt_ud_wrap(wrapped_kde), wrapped_kde)

  unwrapped_kde <- hr_tt_ud_unwrap(wrapped_kde)
  ...
  expect_identical(hr_tt_ud_unwrap(unwrapped_kde), unwrapped_kde)
})
...
test_that("UD storage helpers validate table and raster columns", {
  ...
  expect_error(hr_tt_ud_wrap(invalid_tbl), "x must be an hr_tt_ud_tbl")
  expect_error(hr_tt_ud_unwrap(invalid_tbl), "x must be an hr_tt_ud_tbl")
})

test_that("hr_tt_ud_saveRDS writes a usable wrapped copy", {
  boar_tt <- readRDS(file.path(test_path("testdata"), "wildboar_tt.rds"))
  boar_kde <- hr_tt_kde(boar_tt, res = 50)
  output_file <- tempfile(fileext = ".rds")
  on.exit(unlink(output_file), add = TRUE)

  expect_silent(hr_tt_ud_saveRDS(boar_kde, output_file))
  saved_kde <- readRDS(output_file)

  expect_s3_class(saved_kde, "hr_tt_ud_tbl")
  expect_true(inherits(saved_kde$ud, "PackedSpatRaster_list"))
  expect_false(inherits(boar_kde$ud, "PackedSpatRaster_list"))

  expect_equal(
    as.matrix(hr_tt_ud_sum(saved_kde)$ud[[1]]),
    as.matrix(hr_tt_ud_sum(boar_kde)$ud[[1]])
  )
  expect_equal(
    hr_tt_ud_overlap(saved_kde),
    hr_tt_ud_overlap(boar_kde)
  )
  expect_equal(
    hr_tt_ud_iso(saved_kde, levels = 0.5),
    hr_tt_ud_iso(boar_kde, levels = 0.5)
  )
  ...
  plain_grouped <- boar_kde %>%
    ...
    dplyr::group_by(sex)
  expect_equal(
    lapply(hr_tt_ud_sum(saved_grouped)$ud, terra::values),
    lapply(hr_tt_ud_sum(plain_grouped)$ud, terra::values)
  )
  expect_true(inherits(saved_kde$ud, "PackedSpatRaster_list"))

  loaded_kde <- hr_tt_ud_unwrap(saved_kde)
