...
ade_G_genetta <- kernelUD(
  SpatialPoints(G_genetta),
  h = "href",
  grid = mask_xy_grid,
  kern = "bivnorm"
)
...
# using MASS
mask.xy <- as.data.frame(expand.grid(
  x = seq(0.01, 1, by = 0.01),
  y = seq(0.01, 1, by = 0.01)
))

H <- (sqrt(0.5 * (var(G_genetta[[1]]) + var(G_genetta[[2]])))) *
  (nrow(G_genetta)^-(1 / 6))

MASS_G_genetta <- kde2d(
  G_genetta$V1,
  G_genetta$V2,
  n = c(100, 100),
  h = c(H * 4, H * 4),
...
kde_G_genetta <- kde(G_genetta, eval.points = mask_xy, H = diag(H * 4))
...
# using kernelUD
ade_A_buselaphus <- kernelUD(
  SpatialPoints(A_buselaphus),
  h = "href",
  grid = mask_xy_grid,
  kern = "bivnorm"
)
...
#                        eval.points = mask_xy)
H_bus <- (sqrt(0.5 * (var(A_buselaphus[[1]]) + var(A_buselaphus[[2]])))) *
  (nrow(A_buselaphus)^-(1 / 6))


MASS_A_buselaphus <- kde2d(
  A_buselaphus$V1,
  A_buselaphus$V2,
  n = c(100, 100),
  h = c(H_bus * 4, H_bus * 4),
