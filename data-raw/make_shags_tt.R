...
shags_meta <- shags_meta %>%
  ...
  mutate(
    bird_id = paste0("kb_", substr(bird_id, nchar(bird_id) - 1, nchar(bird_id)))
  )
...
shags_tt_truncated <- shags_tt %>%
  group_by(bird_id) %>%
  mutate(point_number = row_number(), n_points = n()) %>%
  filter(
    bird_id == "kb_38" |
      bird_id == "kb_40" |
      !(bird_id %in%
        c("kb_17", "kb_19", "kb_29", "kb_45") &
        point_number > n_points * 0.5),
    !(bird_id == "kb_27" &
      point_number > n_points * 0.5),
    !(bird_id %in% c("kb_42", "kb_43") & point_number > n_points * 0.5)
  ) %>%
  ungroup() %>%
...
data.frame(
  ...
  points_in_shags_tt = sapply(
    unique(shags_tt$bird_id),
    function(id) sum(shags_tt$bird_id == id)
  ),
  points_in_shags_shags_tt_truncated = sapply(
    unique(shags_tt$bird_id),
    function(id) sum(shags_tt_truncated$bird_id == id)
  )
)
...
df_dups <- df_dups %>%
  ...
  dplyr::mutate(lon_prev = dplyr::lag(lon), lat_prev = dplyr::lag(lat)) %>%
  dplyr::mutate(
    dup_loc = ifelse(lon == lon_prev & lat == lat_prev, TRUE, FALSE)
  ) %>%
  dplyr::ungroup()
...
df_check %>%
  filter(is_penultimate | is_last_point, as.numeric(speed) > 100) %>%
...
show_meta(shags_tt) <- show_meta(shags_tt) %>%
  mutate(
    colony_lon = sf::st_coordinates(colony_coord)[, "X"],
...
shags_tt <- shags_tt %>%
  mutate(
    lon = sf::st_coordinates(shags_tt)[, 1],
...
write.csv(
  shags_df_2,
  file = "inst/extdata/shags_example.csv",
  row.names = FALSE
)
